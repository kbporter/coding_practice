...
tempfile = filepath + 'list' + filename + filetype

with open(tempfile, 'w') as tempwrite:
    dstring = '[' + ", ".join(str(x) for x in dlist) + ']'
    tempwrite.write(dstring) 
    tempwrite.close()
...
totalrows = len(d)
print 'numrows:', totalrows, type(d), type(d[0]), '\n', d[0]
